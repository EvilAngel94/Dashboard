...
class WeatherForeCastCallApiImpl(
    ...
    override fun getWeatherForecast(weatherForeCastApiRequest: WeatherForeCastCallApi.WeatherForeCastApiRequest): WeatherForeCastCallApi.WeatherForeCastApiResponse {
        val apiKey = weatherForeCastApiRequest.apiKey
    ...
    private fun convertResponseToWeatherForecast(re: WeatherForecastResponse, homeLocation: Boolean): WeatherForecast {
        return if (homeLocation) {
            WeatherForecast(
                    homeLocation = re.name,
                    homeTemperature = re.main.temp,
                    homeWeatherType = re.weather.main,
                    homeTemperatureFeelsLike = re.main.feels_like,
                    homeWeatherPicture = re.weather.icon
            )
        } else {
            WeatherForecast(
                    workLocation = re.name,
                    workTemperature = re.main.temp,
                    workWeatherType = re.weather.main,
                    workTemperatureFeelsLike = re.main.feels_like,
                    workWeatherPicture = re.weather.icon
            )
        }
    }
}